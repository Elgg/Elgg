...
function _elgg_notifications_init() {
	...
	elgg_register_page_handler('notice', '_elgg_handle_notice_page');
}
...
/**
 * Forward the user to a page specifically for displaying a message more prominently and
 * persistently than with system_message().
 *
 * The title of the page will be generated from elgg_echo("notice:title:<type>") and
 * this is also passed through the plugin hook [notice_page:filter, title] for dynamic alteration.
 *
 * The page content will be generated from the view "notice/<type>" and the view is passed
 * the type, the URL segments (note: can be altered by the user!), the session data, and the
 * final title.
 *
 * @param string $type         The type of the message. The message will be generated by the view
 *                             "core/notice/<type>".
 * @param string $url_segments If given, this will be appended to the URL after "/notice" and these
 *                             will be available as an array under the key "url_segments" in the view.
 * @param array $session_data  If given, this data will be stored in the session and passed to the
 *                             view under the key "session_data".
 * @param int   $ttl           Number of seconds this notice will be available at the /notice URL
 *
 * @since 1.9
 */
function elgg_forward_to_notice_page($type, $url_segments = '', array $session_data = array(), $ttl = 120) {
	elgg_get_session()->set("notice_page", array(
		'type' => $type,
		'data' => $session_data,
		'expires' => (time() + $ttl),
	));
...
/**
 * Draw informational notice pages
 *
 * @param array $url_segments URL segments
 * @return bool
 *
 * @access private
 */
function _elgg_handle_notice_page($url_segments) {
	$session_data = elgg_get_session()->get("notice_page");
	if (!$session_data) {
		forward('', '404');
	}
	if (time() > $session_data['expires']) {
		elgg_get_session()->remove('notice_page');
		forward('', '404');
	}
	...
	$content_view = "notice/$type";
	if (!elgg_view_exists($content_view)) {
		forward('', '404');
	}
	...
	$title = elgg_echo("notice:title:$type");
	$title = elgg_trigger_plugin_hook('notice_page:filter', 'title', $params, $title);

	$params['title'] = $title;

	$body = elgg_view_layout('notice', array(
