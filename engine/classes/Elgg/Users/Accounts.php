...
class Accounts {
	...
	public function requestNewEmailValidation(\ElggUser $user, $email) {
		...
		$user->setMetadata('new_email', $email);
	...
	public function registerAuthenticationFailure(\ElggUser $user): void {
		$fails = (int) $user->getMetadata('authentication_failures');
		$fails++;

		$user->setMetadata('authentication_failures', $fails);
		$user->setMetadata("authentication_failure_{$fails}", time());
	...
	public function resetAuthenticationFailures(\ElggUser $user): void {
		...
		for ($n = 1; $n <= $fails; $n++) {
			$user->deleteMetadata("authentication_failure_{$n}");
		}

		$user->deleteMetadata('authentication_failures');
	}
	...
	public function isAuthenticationFailureLimitReached(\ElggUser $user, int $limit = null, int $lifetime = null): bool {
		...
		for ($n = $fails; $n > 0; $n--) {
			$failure_timestamp = $user->getMetadata("authentication_failure_{$n}");
