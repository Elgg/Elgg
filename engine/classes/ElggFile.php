...
class ElggFile extends ElggObject {
	...
	public function getDownloadURL($use_cookie = true, $expires = '+2 hours') {
		...
		return _elgg_services()->hook->trigger('download:url', 'file', $params, $url);
	}
	 ...
	 * @return string
	 */
	function getInlineURL($use_cookie = false, $expires = '') {
		$file_svc = new \Elgg\FileService\File();
		$file_svc->setFile($this);
		if ($expires) {
			$file_svc->setExpires($expires);
		...
		return _elgg_services()->hook->trigger('inline:url', 'file', $params, $url);
	}
