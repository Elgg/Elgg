...
class ElggViewService {
	 ...
	 * 
	 * @return ElggDeprecationWrapper|null
	 */
	...
	public function renderView($view, array $vars = array(), $bypass = false, $viewtype = '') {
		...
		foreach ($viewlist as $priority => $view) {
			try {
				$content .= $this->renderViewFile($view, $vars, $viewtype);
				continue;
			} catch (Exception $e) {
			...
			if ($viewtype !== 'default' && $this->doesViewtypeFallback($viewtype)) {
				try {
					$content .= $this->renderViewFile($view, $vars, 'default');
				} catch (Exception $e) {
					$this->logger->log($e->getMessage(), 'NOTICE');
				}
			}
	...
	/**
	 * Includes view PHP file
	 * 
	 * @param string $view     The view name
	 * @param array  $vars     Variables passed to view
	 * @param string $viewtype The viewtype
	 * @throws Exception
	 * @return string output generated by view file inclusion
	 */
