...
class ElggCoreEntityTest extends \Elgg\LegacyIntegrationTestCase {
	...
	public function setUp() {
		...
		$this->entity = $this->createObject([
			'subtype' => $this->getRandomValidSubtype(),
		], [
			'test_metadata' => 'bar',
		]);
	...
	public function testElggEntityGetAndSetAnnotations() {
		...
		$this->assertIdentical($annotations, elgg_get_annotations(['guid' => $this->entity->getGUID(), 'annotation_name' => 'non_existent']));
		$this->assertIdentical($annotations, elgg_get_annotations(['guid' => $this->entity->getGUID(), 'annotation_name' => 'non_existent', 'type' => 'object']));
		$this->assertIdentical(false, elgg_get_annotations(['guid' => $this->entity->getGUID(), 'type' => 'object', 'subtype' => 'fail']));
		...
		$this->assertIdentical([], elgg_get_annotations(['guid' => $this->entity->getGUID(), 'type' => 'object']));
	}
	...
	public function testElggEntityMultipleMetadata() {
		foreach ([$this->entity, new \ElggObject()] as $obj) {
			$md = ['brett', 'bryan', 'brad'];
			$name = 'test_md_' . rand();
	...
	public function testElggEntityAppendMetadata() {
		foreach ([$this->entity, new \ElggObject()] as $obj) {
			...
			$this->assertEqual(['test', 'test2'], $obj->$name);
		}
	}
