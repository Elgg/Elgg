...
class ElggIconMappingIntegrationTest extends ViewRenderingIntegrationTestCase {
	...
	public function testCanViewIcon() {
		...
		$this->assertXmlStringEqualsXmlString($view, '<span class="elgg-icon elgg-icon-mail custom-class fa-regular fa-envelope"></span>');
	}

	public function testCanRenderIconMarkup() {
		$view = elgg_view_icon('abcdefg', ['data-icon' => 'foo']);

		$this->assertXmlStringEqualsXmlString($view, '<span class="elgg-icon elgg-icon-abcdefg fa-solid fa-abcdefg" data-icon="foo"/>');
	}
