...
class ElggCoreRegressionBugsTest extends \Elgg\LegacyIntegrationTestCase {
	...
	public function test_extra_columns_dont_appear_in_attributes() {
		...
		foreach (['site', 'user', 'group', 'object'] as $type) {
	...
	public function test_ElggXMLElement_does_not_load_external_entities() {
		...
		$payload = file_get_contents($this->getTestFilePath('xxe/request.xml'));
		$path = realpath($this->getTestFilePath('xxe/external_entity.txt'));
		...
		if (!$can_load_entity) {
			$this->markTestSkipped("XXE vulnerability cannot be tested on this system");
		}
	...
	public function test_update_handlers_can_change_attributes() {
		...
		elgg_register_event_handler('update', 'object', ['\ElggCoreRegressionBugsTest', 'handleUpdateForIssue6225test']);

		$object->save();

		elgg_unregister_event_handler('update', 'object', ['\ElggCoreRegressionBugsTest', 'handleUpdateForIssue6225test']);
