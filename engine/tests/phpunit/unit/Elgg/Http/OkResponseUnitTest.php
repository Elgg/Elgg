 ...
 * @group HttpService
 */
class OkResponseUnitTest extends UnitTestCase {
	...
	public function statusCodesProvider() {
		...
		foreach (range(100, 599) as $code) {
			$codes[] = [$code, [
				$code >= 100 && $code <= 199, // isInformational
				$code >= 200 && $code <= 299, // isSuccessful
				$code == 200, // isOk
				in_array($code, [201, 301, 302, 303, 307, 308]), // isRedirection
				$code >= 400 && $code <= 499, // isClientError
				$code >= 500 && $code <= 599, // isServerError
				$code == 304, // isNotModified
			]];
		}
		return $codes;
	}
