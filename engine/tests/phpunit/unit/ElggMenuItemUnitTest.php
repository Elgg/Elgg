...
class ElggMenuItemUnitTest extends \Elgg\UnitTestCase {
	...
	public function testFactoryNoNameOrText() {
		$this->assertNull(\ElggMenuItem::factory(['name' => 'test']));
		$this->assertNull(\ElggMenuItem::factory(['text' => 'test']));
	}

	public function testFactoryNoHref() {
		$item = \ElggMenuItem::factory(['name' => 'test', 'text' => 'test']);
	...
	public function testSetContextWithString() {
		...
		$this->assertEquals(['mine'], $item->getContext());
	}
	...
	public function testInContextWithParticularContext() {
		$item = new \ElggMenuItem('name', 'text', 'url');
		$item->setContext(['blog', 'bookmarks']);
		$this->assertTrue($item->inContext('blog'));
	...
	public function testSetLinkClassWithArray() {
		$item = new \ElggMenuItem('name', 'text', 'url');
		$item->setLinkClass(['elgg-link', '2nd-link']);
	...
	public function testArgumentTypeValidationOnItemRegistration() {
		...
		$this->assertTrue(elgg_register_menu_item('foo', ['name' => 'foo', 'text' => 'bar']));
		$this->assertFalse(elgg_register_menu_item('foo', []));
		$this->assertFalse(elgg_register_menu_item('foo', ['text' => 'bar']));
