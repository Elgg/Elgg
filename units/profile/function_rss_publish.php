<?php
/*
 *	Function to publish weblog posts and files as RSS, either a static file or return output
 *	
 ...
 */
	...
	if (isset($parameter) && is_array($parameter)) {
		...
		if ($username) {
			...
			$rssfile = path . "_rss/data/" . $publish_folder . "/" . $username . "/profile.xml";
...
END;
			...
			// WEBLOGS
			$output .= run("weblogs:rss:getitems", array($userid, 10, $tag));
			
			// FILES
			$output .= run("files:rss:getitems", array($userid, 10, $tag));
