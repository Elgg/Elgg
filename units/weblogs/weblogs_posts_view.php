<?php
	...
	if (isset($parameter)) {
		...
		$body = str_replace("{{more}}","<a href=\"/".$username."/weblog/{$post->ident}.html\">More ...</a>",$body);
...
END;
		...
		if ($post->owner == $_SESSION['userid'] && logged_on) {
			...
			<p>
				<small>
					[<a href="/_weblog/edit.php?action=edit&weblog_post_id={$post->ident}">Edit</a>]
					[<a href="/_weblog/action_redirection.php?action=delete_weblog_post&delete_post_id={$post->ident}" onClick="return confirm('Are you sure you want to permanently delete this weblog post?')">Delete</a>]
				</small>
			</p>
...
END;
		...
		$comments = "<a href=\"/".$username."/weblog/{$post->ident}.html\">$numcomments comment(s)</a>";		
		...
		if (isset($individual) && ($individual == 1)) {
			...
			<p>
				<small>
					[<a href="/_weblog/action_redirection.php?action=weblog_comment_delete&weblog_comment_delete={$comment->ident}" onClick="return confirm('Are you sure you want to permanently delete this weblog comment?')">Delete</a>]
				</small>
			</p>
END;
					}
