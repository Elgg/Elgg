	...
	if (isset($_REQUEST['action'])) {
		...
		if (logged_on) {
			...
			switch($_REQUEST['action']) {
						...
						case "content:flag":		if (isset($_REQUEST['address'])) {
													...
													if (isset($_REQUEST['remove'])) {
														...
														if (sizeof($remove) > 0) {
															foreach ($remove as $remove_url) {
																$remove_url = addslashes($remove_url);
																...
																if (trim($new_username[$i]) != "" &&
																	...
																	trim($new_email[$i]) != "") {
																		...
																		if (!preg_match("/^[A-Za-z0-9]{3,12}$/",$new_username[$i])) {
																			...
																			$exists = db_query("select count(ident) as num from users where username = '" . addslashes($new_username[$i]) . "'");
																			$exists = $exists[0]->num;
