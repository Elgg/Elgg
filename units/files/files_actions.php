			...
			switch($_REQUEST['action']) {
					...
					case "files:uploadfile":
												if 	(
														...
														if 	(
																...
																$total_quota = db_query("select sum(size) as sum from files where owner = " . $_SESSION['userid']);
																$total_quota = $total_quota[0]->sum;
																
																$max_quota = db_query("select file_quota from users where ident = " . $_SESSION['userid']);
																$max_quota = $max_quota[0]->file_quota;
																
																if ($total_quota + $_FILES['new_file']['size'] > $max_quota) {
																	$messages[] = "You have exceeded your file quota. You must delete some files before you can upload this one.";
																} else {
																	...
																	$upload_folder = substr($_SESSION['username'],0,1);
																	
																	if (!file_exists(path . "_files/data/" . $upload_folder)) {
																		mkdir(path . "_files/data/" . $upload_folder);
																	}
																	
																	if (!file_exists(path . "_files/data/" . $upload_folder . "/" . $_SESSION['username'])) {
																		mkdir(path . "_files/data/" . $upload_folder . "/" . $_SESSION['username']);
																	}
																	
																	$new_filename = path . "_files/data/" . $upload_folder . "/" . $_SESSION['username'] . "/" . $new_filename;
																	...
																	if (move_uploaded_file($_FILES['new_file']['tmp_name'],$new_filename)) {
																		...
																		if ($_REQUEST['new_file_keywords'] != "") {
																			...
																			if (sizeof($keyword_list) > 0) {
																				foreach($keyword_list as $key => $list_item) {
																					$list_item = addslashes(trim($list_item));
																					db_query("insert into tags set tagtype = 'file', access = '$access', tag = '$list_item', ref = $file_id, owner = " . $_SESSION['userid']);
																				}
																			}
																		}
														...
														define('redirect_url', $redirect_url);
														$file_data = "";
														$file_data->new_file_description = $_REQUEST['new_file_description'];
														$file_data->new_file_title = $_REQUEST['new_file_title'];
														$file_data->new_file_access = $_REQUEST['new_file_access'];
														$file_data->folder = $_REQUEST['folder'];
														$file_data->new_file = $_REQUEST['new_file'];
				...
				// Edit a file
					case "files:editfile":		if (
														...
														$file_info = db_query("select owner from files where ident = $file_id");
														$file_info = $file_info[0];
														if ($file_info->owner == $_SESSION['userid']) {
				...
				// Delete a folder
					case "delete_folder":		if (
														...
														if ($id > -1) {
															$folder = db_query("select * from file_folders where ident = $id and owner = " . $_SESSION['userid']);
															if (sizeof($folder) > 0) {
																$folder = $folder[0];
				...
				// Delete a file
					case "delete_file":		if (
														...
														if ($id > -1) {
															$file = db_query("select * from files where ident = $id and owner = " . $_SESSION['userid']);
															if (sizeof($file) > 0) {
																$file = $file[0];
