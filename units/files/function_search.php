<?php
	...
	if (isset($parameter) && $parameter[0] == "file") {
		...
		if (sizeof($folder_refs) > 0) {
			...
			foreach($folders as $folder) {
				$run_result .= run("templates:draw", array(
									...
									'url' => "/".stripslashes($folder->username)."/files/".$folder->ident,
									'ident' => $folder->ident,
		...
		if (sizeof($file_refs) > 0) {
			...
			foreach($files as $file) {
				$run_result .= run("templates:draw", array(
									...
									'url' => "/".$file->username."/files/".$file->folder."/".$file->ident."/".$file->originalname,
									'menu' => "",
		...
		$body .= <<< END
		<td align="center">
			<a href="/search/index.php?file={$link_keyword}&owner={$friends_userid}">
