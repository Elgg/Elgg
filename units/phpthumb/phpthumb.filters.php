...
class phpthumb_filters {
	...
	function HistogramAnalysis(&$gdimg) {
		...
		$keys = array('red', 'green', 'blue', 'alpha', 'gray');
		foreach ($keys as $key) {
			ksort($Analysis[$key]);
		}
		return $Analysis;
	...
	function ImageBorder(&$gdimg, $border_width, $radius_x, $radius_y, $hexcolor_border) {
		...
		if ($gd_border_canvas = phpthumb_functions::ImageCreateFunction($output_width, $output_height)) {
			...
			$color_background = ImageColorAllocateAlpha($gd_border_canvas, 255, 255, 255, 127);
			...
			ImageAlphaBlending($gdimg, false);
			$gdimg_color_background = ImageColorAllocateAlpha($gdimg, 255, 255, 255, 127);
			...
			switch ($alignment) {
				case '*':
					if ($gdimg_tiledwatermark = phpthumb_functions::ImageCreateFunction($img_source_width, $img_source_height)) {
						...
						$text_color_transparent = ImageColorAllocateAlpha($gdimg_tiledwatermark, 255, 0, 255, 127);
