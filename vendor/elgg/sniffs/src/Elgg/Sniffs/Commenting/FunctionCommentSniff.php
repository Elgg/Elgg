 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class Elgg_Sniffs_Commenting_FunctionCommentSniff implements PHP_CodeSniffer_Sniff
{
    ...
    protected function processParams($commentStart)
    {
        ...
        if (empty($params) === false) {
            ...
            $firstParm = $params[0];
            if (substr_count($firstParm->getWhitespaceBefore(), $this->currentFile->eolChar) !== 2) {
                $error    = 'First parameter comment requires a blank newline before it';
                $errorPos = ($firstParm->getLine() + $commentStart);
                $this->currentFile->addError($error, $errorPos);
            }
