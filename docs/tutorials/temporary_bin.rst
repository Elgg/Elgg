...
When an entity with the restorable capability is deleted, it is marked as soft deleted in the database and hidden from view.
...
The entities with restorable capability are as follows:
...
Giving an entity the restorable capability will reroute its ``delete()`` action
...
An ``elgg_call()`` is performed to retrieve all entities which have the ``restorable`` capability, and which
