...
All collections owned by the user prior to this change have been assigned a subtype of ``friends_collection``.

The groups access collection information was previously stored in the ``group_acl`` metadata.
With the introduction of the ACL subtype this information has been moved to the ACL subtype attribute.

The ``ACCESS_FRIENDS`` level has been migrated to a set of access collections with the subtype ``friends``.
The ``ACCESS_PRIVATE`` level has been migrated to a set of access collections with the subtype ``private``.

All entities and annotations have been updated to use the new access collection ids based on their owner.

Plugins should stop using these global constants and instead use:

 * ``elgg_get_private_access()``
 * ``elgg_get_friends_access()``
 * ``elgg_get_member_access()``

There is some backwards compatibility logic in place that automatically resolves the new access id if the ``owner_guid`` is set,
however the plugins should stop using the global constants to avoid unexpected ownership/access combinations.
