...
Create river items
------------------
``elgg_create_river_item()`` has been deprecated in favor of event-driven river creation.
Plugins should no longer create items dynamically instead they should register for a specific entity, annotation or relationship
event using ``elgg_register_river_event()``, this brings consistency to how river items are create and displayed.
River items no longer use item-specific views, instead plugins should use hooks and partial views to override river elements.

River table now holds information about the result of the event that triggered item creation, subsequently:

* the object of the comment river item is no longer the comment but the item the comment was made on, and the result is the comment entity
* ``annotation_id`` is no longer used, instead any instance of ``ElggData`` can represent the result of the river event
* ``join`` river items renamed to ``member`` to match the relationship name
* ``profileiconupdate`` event is now triggered on upload/crop

These changes make it easier to work with river items:

 * plugins can easily unregister the river event and not have to override action/do other complex things
 * plugins can easily register a river event and easily customize parts of the river item they need to, without having to create a lot of boilerplate copy/paste
 * plugins can export river items in a consistent form (e.g. for use via web services)
...
Entities, relationships, annotations, metadata and river items now all use before and after events for ``create``, ``update`` and ``delete``.
Plugins should start using, e.g. ``create:before`` and ``create:after`` instead of the convoluted legacy ``create`` event.

During entity delete operations, ``delete:before`` and ``delete:after`` events are no longer triggered for individual relationships, annotations, metadata and river items.
Plugins that need to perform clean up bound to these entity properties, should instead listen to entity events.