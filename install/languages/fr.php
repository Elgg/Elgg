...
2. Copier le fichier %s/settings.example.php dans elgg-config/settings.php et suivre les instructions à l\'intérieur du fichier pour définir les paramètres de la base de données. ',
	...
	'install:database:label:dataroot' => 'Répertoire des données',
	'install:settings:label:language' => 'Langue du site',
	'install:settings:label:siteaccess' => 'Accès par défaut au site',
	'install:label:combo:dataroot' => 'Elgg crée un répertoire pour les données',
	...
	'install:settings:help:dataroot:apache' => 'Elgg vous donne la possibilité de créer un répertoire des données ou de renseigner le nom du répertoire que vous avez déjà créé pour stocker les fichiers des utilisateurs (les permissions sur ce répertoire seront vérifiées lorsque vous cliquerez sur Suivant)',
	...
	'install:complete:admin_notice:custom_index' => 'Le plugin Front Page Demo a été activé afin que vous puissiez gérer votre page d\'accueil. Configurez-le ici : %s.',
	...
	'install:error:mysql_version' => 'MySQL doit être en version %s ou supérieure. Votre serveur utilise la version %s.',
	'install:error:database_version' => 'La base de données doit être en version %s ou supérieure. Votre serveur utilise %s.',
	...
	'install:error:relative_path' => 'Nous ne pensons pas que "%s" soit un chemin absolu pour votre répertoire des données',
	'install:error:datadirectoryexists' => 'Votre répertoire des données %s n\'existe pas.',
	'install:error:writedatadirectory' => 'Le serveur web ne peut pas écrire dans votre répertoire des données %s.',
	'install:error:locationdatadirectory' => 'Pour des raisons de sécurité, le répertoire des données %s doit être en dehors du répertoire de l\'installation.',
...
\n\nUne cause moins probable est qu'Apache est configuré avec un alias pour votre répertoire Elgg et que vous deviez alors définir le RewriteBase dans votre fichier .htaccess. Vous trouverez plus d'instructions dans le fichier .htaccess de votre répertoire Elgg.",
	...
	'install:error:rewrite:unknown' => 'Aïe. Nous n\'avons pas réussi à identifier le type de serveur Web utilisé sur votre serveur et cela a fait échouer la mise en place des règles de réécriture. Nous ne pouvons pas vous donner de conseil particulier dans ce cas. Veuillez consulter le lien de dépannage.',
