			...
			if ($friend instanceof ElggUser ) {
				...
				foreach($NOTIFICATION_HANDLERS as $method => $foo) {
					...
					$fields .= <<< END
					    ...
					    <a href="#" border="0" id="{$method}{$friend->guid}" class="{$method}toggleOff" onclick="adjust{$method}_alt('{$method}{$friend->guid}'); return false;">
					    <input type="checkbox" name="{$method}subscriptions[]" id="{$method}checkbox" onclick="adjust{$method}('{$method}{$friend->guid}'); return false;" value="{$friend->guid}" {$checked[$method]} /></a></td>
END;
					$i++;
				}
?>
