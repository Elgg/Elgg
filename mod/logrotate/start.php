...
function log_browser_delete_log($time_of_delete) {
	...
	while ($showtablerow = mysql_fetch_array($result)) {
		$log_time = explode("{$CONFIG->dbprefix}system_log_", $showtablerow[0]);  //To obtain time of archival
		if ($log_time < $ts) {
			if (!mysql_query("DROP TABLE $showtablerow[0]")) {         //If the time of archival is before the required offset then delete
				$FLAG = 0;
			}	
		}
	}
	...
	if ($FLAG) {
		return true;                                                           //Check if the appropriate tables have been deleted and return true if yes
	} else {
		return false;
	}
